						...
						<div class="padding-left">
							<div class="profile-title">
								<div style="margin:20px">
									<h3>Edit Company Details <a href="{{ url()->previous() }}" class="btn btn-primary mx-2 my-2 float-right">Back</a></h3>
								</div>
								<form id="formA7">
									@csrf
								
									<div class="upload-img-bar">
										<img style="width: 150px; border-radius: 50%; margin-right: 50px;" src="{{ asset('images/resource/' . $company->company_image) }}" alt="Company Image" />
...
@push('scripts')
<script>
document.getElementById('formA7').addEventListener('submit', async function (e) {
    e.preventDefault();
    ...
    try {
        const response = await fetch('{{ route("company.update", $company->id) }}', {
            ...
            headers: {
                ...
                'Accept': 'application/json' // Forces Laravel to return JSON for errors
            },
            body: formData
        });
        ...
        try {
            ...
            throw new Error('Invalid JSON from server. Check if the route returns JSON.');
        }
        ...
        Swal.fire({
            icon: 'success',
            title: 'Updated',
            text: result.message || 'Company updated successfully!',
            timer: 2000,
